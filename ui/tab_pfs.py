...
def build_pfs_tab(parent, app):
    ...
    def _on_src_b(*_):
        ...
        # source (so picking a new image refreshes it). Only a name the user
    ...
    btn_row_b = tk.Frame(body_b, bg=COLORS['bg_2'])
    btn_row_b.pack(anchor='w', pady=(14, 0))
    btn_b = make_themed_button(btn_row_b, text='Build .ffpfs now',
    ...
    queue_card = tk.Frame(body_b, bg=COLORS['bg_3'],
    ...
    def _refresh_queue():
        ...
        if not queue:
            ...
            qcount_var.set('')
            return
        queue_card.pack(fill='x', padx=24, pady=(14, 0))
        ...
        for idx, job in enumerate(queue):
            ...
            st = tk.Label(row, text=job['status'], font=FONTS['meta'],
                          ...
                          fg=_STATUS_COLORS.get(job['status'], COLORS['fg_4']))
            st.pack(side='right')
            # Remove button (only when not running/done)
            if job['status'] == 'queued' and not queue_state['running']:
                def _rm(i=idx):
                    queue.pop(i)
                    _refresh_queue()
                tk.Label(row, text='\u2715', font=FONTS['meta'],
                         bg=COLORS['bg_3'], fg=COLORS['fg_5'],
                         cursor='hand2').pack(side='right', padx=(0, 10))
                row.winfo_children()[-1].bind('<Button-1>', lambda e, i=idx: _rm(i))
    ...
    def _validate_job():
        ...
        return {
            ...
            'name':      name,
            'outdir':    outdir,
            'compress':  bool(compress_var.get()),
    ...
    def _build_job(job, on_finish):
        """Run one PFS pack job. Calls on_finish(success: bool) on the Tk
        thread when complete. Updates prog_b live."""
        prog_b.reset()
        ...
        def worker():
            ...
            try:
                ...
                if job['version_ps5']:
                    argv += ['--version', 'PS5']
                argv += ['--cpu-count', str(job['cpu_cores'])]
                ct = job.get('temp_dir', '')
                if ct and os.path.isdir(ct):
                    ...
                    try:
                        ...
                        _tmp = None
                argv += [job['game_root'], job['out_path']]
                ...
                for _ext in ('.ffpfs', '.ffpfsc'):
                    _cand = os.path.normpath(os.path.splitext(job['out_path'])[0] + _ext)
    ...
    def _run_convert():
        ...
        def worker():
            ...
            try:
                ...
                # Preserve empty directories (mkpfs drops them \u2014 breaks
                # runtime path resolution for Unreal and other engines).
                _seed_empty_dir_markers(game_root)
                _log('mkpfs ' + ' '.join(argv))
                ...
                if rc == 0 or (rc == 1 and file_ok):
                    sz = os.path.getsize(actual) / 1024**3
                    def _done_c(s=dict(stats), _sz=sz, _act=actual):
                        prog_c.done()
                        btn_c.config(state='normal')
                        prog_c.set_status('Done \u2713  %.2f GB' % _sz)
