        ...
        def worker():
            ...
            try:
                ...
                if uncompressed:
                    ...
                    # v3.6.2: user-selectable compression level (default 6).
                    # Honour the user's CPU selection as a ceiling, but also
                    # apply the level-scaled worker cap that keeps mkpfs's
                    # unbounded imap backlog in check (prevents the level-7
                    # memory overflow). ShadowMount mounts any level.
                    from exfat_builder import _pfs_pack_tuning
                    _lvl2, _safe_cpu2 = _pfs_pack_tuning(app._settings)
                    _cc2 = max(1, min(int(job['cpu_cores']), _safe_cpu2))
                    argv2 = ['pack', 'file',
                             '--version', 'PS5', '--inode-bits', '32',
                             '--cpu-count', str(_cc2),
                             '--compression-level', str(_lvl2)]
                    _log('mkpfs compression level: %d' % _lvl2)
                    _log('mkpfs CPU count: %d' % _cc2)
                    step2_tmp = tmp_args
                    if tmp_args:
    ...
    def _run_convert():
        ...
        def worker():
            ...
            try:
                ...
                # v3.6.2: user-selectable level (default 6) + level-scaled
                # CPU cap under the user's selection (see step-2 note).
                from exfat_builder import _pfs_pack_tuning
                _lvl9, _safe_cpu9 = _pfs_pack_tuning(app._settings)
                _cc9 = max(1, min(int(_cc), _safe_cpu9))
                argv2 = ['pack', 'file', '--version', 'PS5',
                         '--inode-bits', '32', '--cpu-count', str(_cc9),
                         '--compression-level', str(_lvl9)] \
                        + (['--verify'] if do_verify else []) \
                        + step2_tmp + [pack_src, out_path]
                _log('mkpfs compression level: %d' % _lvl9)
                _log('mkpfs CPU count: %d' % _cc9)
                ...
                _log('mkpfs ' + ' '.join(('"%s"' % a if ' ' in a else a)
                                         for a in argv2))
