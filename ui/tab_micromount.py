...
def _build_toolbar(parent, app):
    ...
    # Icon swatch (teal-soft)
    icon_wrap = tk.Frame(inner, bg=_BG_TOOLBAR)
    icon_wrap.pack(side='left', fill='y')
    icon = tk.Frame(icon_wrap, bg=_TEAL_SOFT, width=40, height=40,
                    highlightthickness=1, highlightbackground=_TEAL_BORDER)
    icon.pack(expand=True)
    icon.pack_propagate(False)
    tk.Label(icon, text='🗂', bg=_TEAL_SOFT, fg=_TEAL,
    ...
    tk.Label(title_row, text='MicroMount',
             bg=_BG_TOOLBAR, fg=_FG, font=_F_H3,
             anchor='w').pack(side='left')
    tk.Label(title_row, text='  \u2014  auto-mount .ffpfsc payload',
    ...
    tk.Label(sub_row, text='      \u2022  4 sections',
    ...
    status_lbl = tk.Label(actions, textvariable=app._mm_status_var,
    ...
    _toolbar_btn(actions, '\U0001f4e1  Send Payload',
                 lambda: _send_payload(app), kind='success').pack(side='right', padx=4)
    _toolbar_btn(actions, '\U0001f4e5  Load from PS5',
                 lambda: _load_from_ps5(app), kind='default').pack(side='right', padx=4)
    _toolbar_btn(actions, '\U0001f4dd  Fetch log',
                 lambda: _fetch_debug_log(app), kind='ghost').pack(side='right', padx=4)
