...
def _build_image_card(parent, app):
    ...
    # 📦 icon tile
    disc = tk.Frame(inner, bg=COLORS['accent_08'], width=44, height=44,
                    highlightbackground=COLORS['accent_lo'],
                    highlightthickness=1)
    disc.pack(side='left')
    disc.pack_propagate(False)
    tk.Label(disc, text='\U0001f4e6',
    ...
    app._fe_card_ppsa = tk.Label(row1, text='',
    ...
    app._fe_card_title = tk.Label(row1, text=_('No image loaded'),
        font=('Segoe UI', 12, 'bold'),
        bg=COLORS['bg_2'], fg=COLORS['fg_0'], anchor='w')
    app._fe_card_title.pack(side='left', padx=(0, 6))
    app._fe_card_path = tk.Label(main, textvariable=app._fe_image_var,
        font=FONTS['mono_sm'],
        bg=COLORS['bg_2'], fg=COLORS['fg_5'], anchor='w')
    app._fe_card_path.pack(fill='x', pady=(2, 0))

    # Buttons column
    btns = tk.Frame(inner, bg=COLORS['bg_2'])
    btns.pack(side='right')
    _ghost_btn(btns, _('Browse...'),
...
def _build_toolbar(parent, app):
    ...
    def _tb(text, cmd, kind='ghost'):
        return _icon_btn(btns, text, cmd, kind=kind)

    _tb('\u2191  ' + _('Up'),         lambda: _go_up(app)
        ).pack(side='left', padx=(0, 4))
    _tb('\u21bb  ' + _('Refresh'),    lambda: _refresh(app)
        ).pack(side='left', padx=(0, 4))
    tk.Frame(btns, bg=COLORS['border_3'], width=1
             ).pack(side='left', fill='y', padx=6)
    _tb('\u2795  ' + _('Add files'),  lambda: _add_files(app), kind='accent'
        ).pack(side='left', padx=(0, 4))
    _tb('\U0001f4c1  ' + _('Add folder'), lambda: _add_folder(app)
        ).pack(side='left', padx=(0, 4))
    _tb('\U0001f4c2  ' + _('New folder'), lambda: _new_folder(app)
        ).pack(side='left', padx=(0, 4))
    # ── Backport apply (overlays a user-made backport folder onto the
    #    open image, backing up originals before any replace). Accent so
    #    it stands out from the plain add/replace buttons. ──
    _tb('\U0001f4e5  ' + _('Apply backport'),
        lambda: _apply_backport(app), kind='accent'
        ).pack(side='left', padx=(0, 4))
    # ── Compare ffpkg (diagnostic — for figuring out why a rebuilt
    #    image differs from a known-good one). Plain ghost button so it
    #    doesn't compete with the apply-backport action. ──
    _tb('\U0001f50d  ' + _('Compare'),
        lambda: _compare_ffpkgs(app)
        ).pack(side='left', padx=(0, 4))
    tk.Frame(btns, bg=COLORS['border_3'], width=1
             ).pack(side='left', fill='y', padx=6)
    _tb('\u21bb  ' + _('Replace'),    lambda: _replace_file(app)
        ).pack(side='left', padx=(0, 4))
    _tb('\u270f  ' + _('Rename'),     lambda: _rename(app)
        ).pack(side='left', padx=(0, 4))
    _tb('\U0001f5d1  ' + _('Delete'), lambda: _delete(app), kind='danger'
        ).pack(side='left', padx=(0, 4))
...
def _build_inspector(parent, app):
    ...
    app._fe_insp_hex_btn = tk.Button(hex_outer,
        text='\U0001f50d  ' + _('Show first 64 bytes'),
...
def _clear_inspector(app):
    if hasattr(app, '_fe_insp_icon'):
        ...
        app._fe_insp_name.config(text=_('No selection'))
...
def _refresh(app):
    ...
    app._fe_status_var.set(_('%d items') % (len(entries) - (1 if path != '/' else 0)))
    _clear_inspector(app)
...
def _on_select(app):
    sel = app._fe_listbox.curselection()
    app._fe_sel_pill.config(text='%d selected' % len(sel))
    if not sel:
        _clear_inspector(app)
        return
    idx = sel[0]
    ...
    app._fe_insp_name.config(text=name)
