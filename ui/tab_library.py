...
def _build_page_head(body, app):
    ...
    # Count pill — bound to status_var via the existing callback chain.
    # The mock shows "47 games · 3 folders"; we just show the text the
    # status_var carries (the existing code sets it in _lib_scan_done).
    # Until a scan runs the pill shows "0 games".
    app._lib_count_var = tk.StringVar(value='0 games')
    count_pill = tk.Label(head, textvariable=app._lib_count_var,
                          font=FONTS['mono_sm'],
...
def _build_toolbar(body, app):
    ...
    # ── Search input ──
    search_wrap = tk.Frame(inner, bg=COLORS['bg_0'],
    ...
    tk.Entry(search_wrap, textvariable=app._lib_search_var,
             font=FONTS['body'],
    ...
    seg = tk.Frame(inner, bg=COLORS['bg_0'],
                   highlightbackground=COLORS['border_3'],
                   highlightthickness=1)
    seg.pack(side='left', padx=(10, 0), ipady=2)
