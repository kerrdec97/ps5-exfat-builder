...
def _build_selected_banner(parent, app):
    """Banner showing the active game's metadata; mirrors the mock's
    `.selected` block. Hidden via pack_forget until a game is selected."""
    bg = '#1a1024'  # purple-tinted dark — tk has no gradients
    border = COLORS['purple']
    banner = tk.Frame(parent, bg=bg,
                      highlightbackground=border,
                      highlightthickness=1)
    # Note: we DO NOT pack here. The banner becomes visible only when
    # update_selected_banner() is called with a game. This avoids an empty
    # purple box at the top of the form.

    inner = tk.Frame(banner, bg=bg)
    inner.pack(fill='x', padx=14, pady=10)

    # Cover thumbnail (left)
    cover = tk.Label(inner, bg='#2d1a3d',
                     width=4, height=2,
                     text='\U0001f3ae',
                     ...
                     highlightthickness=1)
    cover.pack(side='left', padx=(0, 14))

    # Info column (middle, expands)
    info = tk.Frame(inner, bg=bg)
    info.pack(side='left', fill='x', expand=True)

    title_var = tk.StringVar(value='No game selected')
    tk.Label(info, textvariable=title_var,
             font=(FONTS['body'][0], 12, 'bold'),
             bg=bg, fg=COLORS['fg_0'], anchor='w'
             ).pack(fill='x')

    # Meta row — CUSA · version · SDK · size · file count
    ...
    # Badges (right) — FW tier + Backportable
    badges = tk.Frame(inner, bg=bg)
    badges.pack(side='right')

    fw_badge = tk.Label(badges, text='', padx=8, pady=2,
                        font=(FONTS['mono_sm'][0], 9, 'bold'))
    fw_badge.pack(side='left', padx=(0, 4))

    bp_badge = tk.Label(badges,
                        text='\u26a1 Backportable',
                        bg=COLORS['purple'], fg=COLORS['fg_0'],
                        font=(FONTS['mono_sm'][0], 9, 'bold'),
                        padx=8, pady=2)
    bp_badge.pack(side='left')

    # Stash widgets/vars on app so update_selected_banner can recolor them
    app._abp_banner          = banner
    app._abp_banner_cover    = cover
    app._abp_banner_title    = title_var
    app._abp_banner_meta     = meta_var
    app._abp_banner_fw_badge = fw_badge


def update_selected_banner(app, game):
    """Refresh the selected-game banner from a game dict.

    Called by `_bp_make_game_card._select` after picking a game. If `game`
    is None the banner is hidden; otherwise it's packed at the top of the
    form with the game's metadata.
    """
    banner = getattr(app, '_abp_banner', None)
    if banner is None:
        return
    if not game:
        try:
    ...
    # Pack at the top of the form. Use `before=` to ensure correct order
    # if the banner was previously hidden.
    try:
        # Pack as the first child of its parent
        banner.pack(fill='x', pady=(0, 12),
                    before=banner.master.winfo_children()[0]
                    if banner.master.winfo_children() and
                       banner.master.winfo_children()[0] is not banner
                    else None)
    except Exception:
        # Fallback: just pack at the end (correct for empty parent)
        banner.pack(fill='x', pady=(0, 12))

    # Update title and meta
    app._abp_banner_title.set(game.get('title', 'Unknown'))
    parts = []
    ...
    if sys_ver:
        parts.append('SDK %d.00' % sys_ver)
    if game.get('size', 0) > 0:
        sz = game['size']
        if sz >= 1024**3:
            parts.append('%.1f GB' % (sz / 1024**3))
        else:
            parts.append('%d MB' % (sz // 1024**2))
    app._abp_banner_meta.set('  \u00b7  '.join(parts))

    # Update FW badge color/text
    if sys_ver:
        if sys_ver <= 4:
            bg, fg = '#1a3a5c', COLORS['accent_hi']
        elif sys_ver <= 7:
            bg, fg = '#2a2a1a', COLORS['warn_hi']
        else:
            bg, fg = '#3a1a1a', COLORS['danger_hi']
        app._abp_banner_fw_badge.configure(
            text=' FW %d.xx+ ' % sys_ver, bg=bg, fg=fg)
    else:
        app._abp_banner_fw_badge.configure(text='', bg=banner.cget('bg'))

    # Async cover load — same pattern as the game cards
    if game.get('cover'):
        def _load(path=game['cover'], lbl=app._abp_banner_cover):
            try:
                from exfat_builder import _load_cover_image
                img = _load_cover_image(path, target=54)
                if img is None:
                    return
                from PIL import ImageTk
                photo = ImageTk.PhotoImage(img)
                lbl.config(image=photo, text='', width=54, height=54)
                lbl._photo = photo
            except Exception:
                pass
        app.after(10, _load)
