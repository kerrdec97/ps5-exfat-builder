...
def build_shadowmount_tab(parent, app):
    ...
    # Activate the overview dashboard by default (v3.6.0 pass)
...
def _build_toolbar(parent, app):
    ...
    hero card (icon tile + title + config stats strip + actions row).

    Backwards compat preserved exactly:
      - app._smp_dirty_pill   Label, packed/unpacked by _refresh_dirty_ui
      - app._smp_status_var   StringVar shown in the hero
      - app._smp_status_lbl   Label whose fg _set_status() reconfigures
    All four actions call the same functions as before. The editor
    ...
    hero = ControlHero(
        wrap,
        title='ShadowMountPlus',
        subtitle=('Manage ShadowMount+ configuration and deployment  '
                  '\u00b7  ' + REMOTE_CONFIG_PATH),
    ...
    # Per-image rules count — read from the editor row once the
    # sections have built (display only, refreshed on a slow poll).
    def _poll_rules():
        try:
            if not wrap.winfo_exists():
                return
            row = (getattr(app, '_smp_rows', {}) or {}).get(
                'per_image_rules')
            if row is not None:
                val = (row.get_value() or '').strip()
                n = len([ln for ln in val.splitlines() if ln.strip()])
                hero.set_stat('rules', str(n))
        except Exception:
            return
        try:
            wrap.after(3000, _poll_rules)
        except Exception:
            pass
    wrap.after(900, _poll_rules)

    hero.set_stat('sections', str(len(_SECTIONS)))
    hero.set_stat('port', str(app._settings.get('smp_payload_port',
                                                '9021') or '9021'))
    hero.set_stat('config', 'Local defaults')
    hero.set_stat('status', 'Idle')
    hero.set_badge('LOCAL', 'wait')

    # ── Status mirror — _set_status() writes _smp_status_var; the
    # hero's STATUS cell shows a trimmed copy.
    def _sync_status(*_a):
        try:
            t = app._smp_status_var.get().strip()
            low = t.lower()
            warn = ('fail' in low or 'error' in low or '\u2717' in t)
            ok = ('\u2713' in t)
            hero.set_stat('status', (t[:26] + '\u2026') if len(t) > 27
                          else (t or 'Idle'), warn=warn, ok=ok)
        except Exception:
            pass
    app._smp_status_var.trace_add('write', _sync_status)

    # ── Config-source mirror — the rail footer var is created after
    # this hero (rail builds later), so hook up deferred.
    def _hook_footer(tries=0):
        var = getattr(app, '_smp_load_footer_var', None)
        if var is None:
            if tries < 10:
    ...
    make_themed_button(actions, '\u26a1  Send Payload',
                       command=lambda: _send_payload(app), kind='success',
                       font_size=9, padx=12, pady=6
                       ).pack(side='left', padx=(6, 0))
    make_themed_button(actions, '\u23cf  Safe to unplug',
                       command=lambda: _stop_shadowmount(app), kind='ghost',
                       font_size=9, padx=12, pady=6
                       ).pack(side='left', padx=(6, 0))

    # Dirty pill — shown when there are pending edits, hidden otherwise.
    # _refresh_dirty_ui() packs it with side='right' into its parent,
    # so it lands at the right edge of the actions row.
    ...
    # Status label (network operation feedback) — full untrimmed text,
    # right-aligned in the hero title row next to the badge.
    status_lbl = tk.Label(hero.title_row, textvariable=app._smp_status_var,
    ...
    # Bottom hairline under the hero zone
...
def _build_rail(parent, app):
    ...
    # Section list — Overview dashboard row first (v3.6.0 pass)
    ov_row = _make_rail_row(parent, app, 'overview', '\U0001f3e0',
                            'Overview', len(_SECTIONS))
    app._smp_rail_rows['overview'] = ov_row
...
def _select_section(app, key):
    ...
    # Overview tiles mirror live editor values — refresh on show
    if key == 'overview':
        fn = getattr(app, '_smp_overview_refresh', None)
        if callable(fn):
            try:
                fn()
            except Exception:
...
def _overview_summary_keys(section_key, limit=3):
    """First few config keys belonging to a section, with prettified
    labels — drives the overview tiles' summary lines."""
    out = []
    for k, sec in _KEY_TO_SECTION.items():
        if sec == section_key:
            label = k
            for pre in ('kstuff_', 'mount_', 'scan_', 'fakelib_',
                        'per_image_', 'smp_'):
...
def _build_overview_card(parent, app):
    """v3.6.0 PS5 pass: dashboard landing section — one summary tile
    per configuration section (click opens the existing editor) plus a
    Recent Activity panel. Values are read from the existing
    SettingRow instances; nothing is written."""
    ...
    tk.Label(hi, text='\U0001f3e0', bg=_BG_CARD_HEAD, fg=_PURPLE,
    ...
    def _tile(r, c, key, icon, label, sublabel, count, accent):
        ...
        def _hin(_e=None, w=t):
            w.config(highlightbackground=_PURPLE)
        def _hout(_e=None, w=t):
            w.config(highlightbackground=_BORDER)
        t.bind('<Enter>', _hin)
        t.bind('<Leave>', _hout)
        return t

    for i, (key, icon, label, sublabel, count, accent_token) in \
            enumerate(_SECTIONS):
        _tile(i // 2, i % 2, key, icon, label, sublabel, count,
              COLORS.get(accent_token, _PURPLE))
    ...
    for k, label in [('payload', 'Last payload send'),
        ...
        tk.Label(r, textvariable=v, bg=COLORS['bg_3'],
                 fg=COLORS['teal'], font=(_F_MONO[0], 9, 'bold'),
    ...
    def _on_status(*_a):
        ...
        try:
            ...
            pass
    try:
        app._smp_status_var.trace_add('write', _on_status)
    except Exception:
        pass
    ...
    def _refresh():
        ...
        for k, lbl in value_setters.items():
            try:
                ...
                pass
    app._smp_overview_refresh = _refresh
...
def _build_content(parent, app):
    ...
    # Overview dashboard section (v3.6.0 pass) — default landing
    app._smp_section_frames['overview'] = _build_overview_card(pad, app)
...
# v3.6.2: confirmed-stop handshake parameters.
_STOP_CONFIRM_TIMEOUT = 45       # seconds to wait for klog confirmation
_STOP_KLOG_CONNECT_TIMEOUT = 5   # seconds to establish the klog socket
#
# Precise ShadowMountPlus stop markers, pinned from a real v1.6 stop
# capture (klog + shadowmount-debug.log). The clean stop sequence is:
#   [SHUTDOWN] stop requested during scanner wait      <- SM+ ACK of STOP
#   [GAME] lifecycle watcher stopped
#   [KSTUFF] removed shutdown sentinel: ...
#   [IMG][LVD] unmount start: ... unit=N               <- one per mount
#   [IMG][LVD] unmount complete: ... unit=N            <- one per mount
#   ...
#   # process pid=NN, shadowmountplus.elf calls exit() exit_value=0.  <- DONE
#
# The kernel-level "shadowmountplus.elf calls exit()" line is the
# gold-standard completion proof: SM+ only exits after every LVD unit is
# released, so seeing it means all mounts are gone and the ELF has
# terminated cleanly. We treat that as definitive confirmation. As a
# secondary path (in case the ELF lingers or the exit line isn't on this
# klog feed), we also confirm once we've seen the [SHUTDOWN] ack AND at
# least one unmount-complete with start-count == complete-count.
_STOP_ACK_MARKER       = '[shutdown] stop requested'
_STOP_EXIT_MARKER      = 'shadowmountplus.elf calls exit'
_STOP_UNMOUNT_START    = 'unmount start'
_STOP_UNMOUNT_COMPLETE = 'unmount complete'


def _stop_shadowmount(app):
    """v3.6.2: confirmed-stop handshake.

    The old flow wrote /data/shadowmount/STOP, slept a blind 3 seconds,
    and showed a green "safe to unplug" — but never verified the ELF saw
    the sentinel or finished releasing mounts (cleanup runs "a second or
    two PER MOUNT", so 3s loses to any real library; and a crashed /
    never-sent ELF ignores the sentinel entirely while the dialog still
    claims safe). Field result: intermittent kernel panics on unplug.

    New flow:
      1. Connect a TEMPORARY klog socket FIRST (saved klog_port, default
         3232) so fast cleanup lines can't be missed. This socket is
         private to the handshake — the Klog tab's own connection, if
         running, is untouched.
      2. Write the STOP sentinel via FTP (always, klog or not).
      3. Watch klog up to _STOP_CONFIRM_TIMEOUT seconds for SM+ cleanup
         confirmation, using precise markers from a real stop capture:
         definitively the kernel "shadowmountplus.elf calls exit()" line,
         or (secondary) the "[SHUTDOWN] stop requested" ack followed by
         every started LVD unmount completing. A live "released N/M
         mounts" status ticks while waiting.
      4. GREEN dialog ONLY on seeing confirmation. Klog unreachable,
         klog dropped mid-wait, or timeout -> AMBER warning: STOP was
         sent but safety is unconfirmed. Green is never shown on faith.

    Re-mounting later still means re-sending the ELF payload (the ELF
    clears the stale STOP file at startup, so no manual cleanup).
    """
    ip = app._ftp_ip_var.get().strip()
    if not ip:
        messagebox.showwarning('No IP',
    ...
    if not messagebox.askyesno('Stop ShadowMountPlus',
            ...
            'The app will watch the PS5 kernel log and only report '
            '\u201csafe to unplug\u201d once ShadowMountPlus confirms its '
            'cleanup finished.\n\n'
    ...
    try:
        klog_port = int(app._settings.get('klog_port', 3232))
    except Exception:
        klog_port = 3232
    ...
    def worker():
        klog_sock = None
        try:
            # 1. klog FIRST, so no cleanup line can race past us while
            #    the sentinel is being written.
            app.after(0, _set_status, app,
                      'Connecting to klog (%s:%d)...' % (ip, klog_port),
                      ACCENT)
            try:
                klog_sock = socket.create_connection(
                    (ip, klog_port), timeout=_STOP_KLOG_CONNECT_TIMEOUT)
                klog_sock.settimeout(1.0)   # recv poll tick
            except Exception:
                klog_sock = None

            # 2. Drop the STOP sentinel (always — klog or not).
            app.after(0, _set_status, app,
                      'Sending STOP signal to PS5...', ACCENT)
            ftp = app._ftp_connect()
            try:
            ...
            if klog_sock is None:
                app.after(0, _on_stop_unconfirmed, app, 'no_klog')
                return

            # 3. Watch for the cleanup confirmation using precise SM+
            #    markers. Definitive: the kernel "shadowmountplus.elf calls
            #    exit()" line. Secondary: [SHUTDOWN] ack seen AND every
            #    started unmount completed (>=1). Surface a live released
            #    N/M counter so the wait is legible.
            confirmed = False
            klog_lost = False
            saw_ack = False
            n_start = 0
            n_done = 0
            start = time.time()
            deadline = start + _STOP_CONFIRM_TIMEOUT
            sbuf = ''
            last_tick = -1
            while time.time() < deadline:
                ...
                if el != last_tick:
                    last_tick = el
                    if saw_ack or n_start:
                        _msg = ('ShadowMountPlus stopping \u2014 released '
                                '%d/%d mounts... %ds' % (n_done, n_start, el))
                    else:
                        _msg = ('Waiting for ShadowMountPlus confirmation'
                                '... %ds' % el)
                    app.after(0, _set_status, app, _msg, ACCENT)
                try:
                    data = klog_sock.recv(4096)
                except socket.timeout:
                    continue
                except Exception:
                    klog_lost = True
                    break
                if not data:        # peer closed the klog stream
                    klog_lost = True
                    break
                ...
                while '\n' in sbuf:
                    ...
                    low = line.lower()
                    # Gold-standard: the ELF terminated -> all units freed.
                    if _STOP_EXIT_MARKER in low:
                        confirmed = True
                        break
                    if _STOP_ACK_MARKER in low:
                        saw_ack = True
                    # Only count unmounts that occur AFTER the stop ack, so
                    # a prior game's unmount lines lingering in the klog
                    # buffer can't be mistaken for this stop's cleanup.
                    if saw_ack and _STOP_UNMOUNT_START in low:
                        n_start += 1
                    if saw_ack and _STOP_UNMOUNT_COMPLETE in low:
                        n_done += 1
                    # Secondary: ack seen and every started unmount done.
                    if saw_ack and n_start > 0 and n_done >= n_start:
                        confirmed = True
                        break
                if confirmed:
                    break
            ...
            if confirmed:
                app.after(0, _on_stop_confirmed, app)
            else:
                app.after(0, _on_stop_unconfirmed, app,
                          'klog_lost' if klog_lost else 'timeout')
        except Exception as e:
            short, title, body = _friendly_ftp_error('Stop', e)
            app.after(0, _set_status, app, short, DANGER)
            app.after(0, messagebox.showerror, title, body)
        finally:
            # Always release the temporary klog socket — confirmation,
            # timeout, klog loss, and FTP error all land here. The Klog
            # tab's own socket (app._klog_socket) is never touched.
            if klog_sock is not None:
                try:
                    klog_sock.close()
                except Exception:
                    pass
...
def _on_stop_confirmed(app):
    """GREEN path: a ShadowMountPlus cleanup line was seen in klog."""
    _set_status(app,
        'ShadowMountPlus cleanup confirmed \u2713 \u2014 safe to unplug',
        SUCCESS)
    messagebox.showinfo('Safe to unplug',
        'ShadowMountPlus cleanup confirmed. It is now safe to unplug '
        'the drive.\n\n'
...
def _on_stop_unconfirmed(app, reason):
    """AMBER path: STOP was sent, but cleanup was NOT confirmed.

    reason: 'no_klog'   — klog socket could not be opened at all
            'timeout'   — klog watched for the full window, no match
            'klog_lost' — klog connected but dropped before a match
    """
    _set_status(app,
        'STOP sent \u2014 cleanup NOT confirmed. Do not unplug yet.',
        WARNING)
    if reason == 'no_klog':
        messagebox.showwarning('STOP sent — not confirmed',
            'STOP signal sent, but cleanup could not be confirmed '
            'because klog is unavailable.\n\n'
            'Wait at least 5 seconds per mounted game and make sure the '
            'USB/enclosure activity light is idle before unplugging.')
    else:
        extra = ''
        if reason == 'klog_lost':
            extra = ('\n\n(The klog connection dropped while waiting, so '
                     'confirmation may have been missed.)')
        messagebox.showwarning('STOP sent — not confirmed',
            'STOP signal sent, but ShadowMountPlus cleanup was not '
            'confirmed.\n\n'
            'ShadowMountPlus may not be running, may have crashed, or '
            'cleanup may still be in progress. Do not unplug yet unless '
            'the drive/enclosure LED is idle.\n\n'
            'Check the Klog tab or resend the ShadowMountPlus payload.'
            + extra)
