...
def build_settings_tab(parent, app):
    ...
    section_specs = [
        ...
        ('theme',         '\U0001f3a8   Theme',
         _('Theme'),
         _('The app ships with a dark theme. More variants are planned.')),
...
def _build_build(body, app):
    ...
    # ── EXTRACTION card (issue #38): optional robocopy multithread ──
    # Applies ONLY to .exfat image extraction (OSFMount mount + robocopy).
    # The ffpkg/PFS (UFS2Tool/Dokan) extraction path is unaffected.
    card_ext = SettingsCard(body, title=_('Extraction (.exfat images)'))
    card_ext.pack(fill='x', pady=(0, 14))

    _r, ctrl, _act = settings_row(card_ext.body,
        _('Use Robocopy multithread extraction'),
        description=_('When extracting an .exfat image to a folder, copy '
                      'multiple files in parallel with robocopy /MT. Off by '
                      'default (Classic single-thread extraction). Faster on '
                      'NVMe/SSD destinations; on slow or USB drives the '
                      'classic single-thread mode is usually safer. Windows '
                      'only. The .ffpkg/.ffpfs/.ffpfsc extraction path is '
                      'not affected by this setting.'),
        with_divider=False)
    if not hasattr(app, '_exfat_extract_mt_var'):
        app._exfat_extract_mt_var = tk.BooleanVar(
            value=app._settings.get('exfat_extract_robocopy_mt', False))

    def _save_extract_mt_pref():
        app._settings['exfat_extract_robocopy_mt'] = bool(
            app._exfat_extract_mt_var.get())
        try:
            save_settings(app._settings)
        except Exception:
            pass

    tk.Checkbutton(ctrl, text=_('Extraction multithread (/MT)'),
                   variable=app._exfat_extract_mt_var,
                   font=FONTS['body'],
                   bg=COLORS['bg_2'], fg=COLORS['fg_3'],
                   activebackground=COLORS['bg_2'],
                   activeforeground=COLORS['fg_1'],
                   selectcolor=COLORS['bg_4'], cursor='hand2',
                   command=_save_extract_mt_pref).pack(side='left')

    _r2, ctrl2, _act2 = settings_row(card_ext.body,
        _('Extraction copy threads (/MT:N)'),
        description=_('Number of parallel copy threads (robocopy /MT:N) when '
                      'multithread extraction is enabled. Default 32. Valid '
                      'range 1\u2013128. Separate from Build Copy Threads.'),
        with_divider=False)
    if not hasattr(app, '_exfat_extract_threads_var'):
        app._exfat_extract_threads_var = tk.StringVar(
            value=str(app._settings.get('exfat_extract_mt_threads', 32)))

    _thr_entry = tk.Entry(ctrl2, textvariable=app._exfat_extract_threads_var,
                          font=FONTS['mono'] if 'mono' in FONTS else FONTS['body'],
                          bg=COLORS['bg_4'], fg=COLORS['fg_1'],
                          insertbackground=COLORS['fg_1'],
                          relief='flat', width=6)
    _thr_entry.pack(side='left')

    def _save_extract_threads(*_a):
        raw = app._exfat_extract_threads_var.get().strip()
        try:
            n = int(raw)
        except Exception:
            n = 32
        n = max(1, min(128, n))
        # Normalise the field if the user typed something out of range/garbage.
        if str(n) != raw:
            try:
                app._exfat_extract_threads_var.set(str(n))
            except Exception:
                pass
        app._settings['exfat_extract_mt_threads'] = n
        try:
            save_settings(app._settings)
        except Exception:
            pass

    _thr_entry.bind('<FocusOut>', _save_extract_threads)
    _thr_entry.bind('<Return>', _save_extract_threads)

    # ── EXPERIMENTAL card (OFF by default; easy to remove) ──
    # Remove this whole card + ui/experimental_mkpfs.py + the pipeline branch
    # to fully revert the external mkpfs-compatible tool path.
    card_exp = SettingsCard(body, title=_('Experimental'))
    card_exp.pack(fill='x', pady=(0, 14))

    _r, ctrl, _act = settings_row(card_exp.body,
        _('Use external mkpfs-compatible tool'),
        description=_('Use an external mkpfs-compatible tool for testing '
                      'alternative PFS packers. This is optional, off by '
                      'default, and only applies to supported test routes '
                      'such as existing .exfat \u2192 .ffpfsc. Stable bundled '
                      'mkpfs remains the default.'),
        with_divider=False)
    if not hasattr(app, '_exp_mkpfs_var'):
        app._exp_mkpfs_var = tk.BooleanVar(
            value=app._settings.get('experimental_mkpfs_enabled', False))

    def _save_exp_mkpfs_pref():
        app._settings['experimental_mkpfs_enabled'] = bool(
            app._exp_mkpfs_var.get())
        try:
            save_settings(app._settings)
        except Exception:
            pass

    tk.Checkbutton(ctrl, text=_('Use external mkpfs-compatible tool'),
                   variable=app._exp_mkpfs_var,
                   font=FONTS['body'],
                   bg=COLORS['bg_2'], fg=COLORS['fg_3'],
                   activebackground=COLORS['bg_2'],
                   activeforeground=COLORS['fg_1'],
                   selectcolor=COLORS['bg_4'], cursor='hand2',
                   command=_save_exp_mkpfs_pref).pack(side='left')

    _r2, ctrl2, _act2 = settings_row(card_exp.body,
        _('External mkpfs tool folder'),
        description=_('Select the folder containing the external '
                      'mkpfs-compatible tool or launcher.'),
        with_divider=False)
    if not hasattr(app, '_exp_mkpfs_dir_var'):
        app._exp_mkpfs_dir_var = tk.StringVar(
            value=app._settings.get('experimental_mkpfs_folder', ''))

    _exp_entry = tk.Entry(ctrl2, textvariable=app._exp_mkpfs_dir_var,
                          font=FONTS['mono'] if 'mono' in FONTS else FONTS['body'],
                          bg=COLORS['bg_4'], fg=COLORS['fg_1'],
                          insertbackground=COLORS['fg_1'],
                          relief='flat', width=40)
    _exp_entry.pack(side='left', fill='x', expand=True, padx=(0, 8))

    def _save_exp_dir(*_a):
        app._settings['experimental_mkpfs_folder'] = \
            app._exp_mkpfs_dir_var.get().strip()
        try:
            save_settings(app._settings)
        except Exception:
            pass
    ...
    def _browse_exp_dir():
        d = filedialog.askdirectory(
            title=_('Select the external mkpfs tool folder'))
        if d:
            app._exp_mkpfs_dir_var.set(d)
            _save_exp_dir()

    app._exp_mkpfs_dir_var.trace_add('write', _save_exp_dir)
    _accent_btn(ctrl2, _('Browse\u2026'), command=_browse_exp_dir).pack(
        side='left')
...
def _build_theme(body, app):
    ...
    # v3.6.3: only the dark variant ships now (see below), so column 1 is
    # no longer weighted — the single card fills the row.
    ...
    # v3.6.3: the light variant was removed. The app's UI is built on the
    # COLORS{} design-token system (2,500+ usages); the old _toggle_theme
    # only repainted a handful of legacy globals, so "Light" produced a
    # broken half-dark result. A proper light theme (a full light token
    # palette) is planned for a future release rather than shipping a
    # misleading half-working toggle.
