...
def build_unibuild_tab(parent, app):
    ...
    def _refresh_via():
        if out_type.get() == 'pfs':
            via_frame.pack(anchor='w', pady=(12, 0))
        else:
    ...
    def _refresh_strip():
        if out_type.get() == 'pfs':
            stat_comp_var.set('.ffpfsc (compressed)  \u00b7  intermediate via %s'
    ...
    def _gen_name(*_):
        ...
        try:
            ...
            base = sanitize_filename(base) or os.path.basename(folder.rstrip('/\\'))
        except Exception:
            pass
    ...
    def _update_space(*_):
        ...
        def _w():
            try:
                size = app._get_folder_size(folder)
            except Exception:
                size = 0
            mult = 2.05 if t == 'pfs' else 1.12
    ...
    def _uq_render():
        ...
        for idx, job in enumerate(queue):
            ...
            label = job['name']
            if job['type'] == 'pfs':
                label += '   via %s' % _TYPE_LABEL[job.get('via', 'exfat')]
    ...
    def _uq_add():
        ...
        queue.append({
            ...
            'via': pfs_via.get() if t == 'pfs' else None,
