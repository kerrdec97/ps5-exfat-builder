...
def build_unibuild_tab(parent, app):
    ...
    def _make_type_card(col, key, icon, title, sub, recommended=False,
        ...
        # v3.6.3: concise hover tooltip explaining each format.
        if tip:
            try:
                from exfat_builder import Tooltip
                for w in [card] + labels:
                    Tooltip(w, tip)
            except Exception:
                pass
    ...
    _make_type_card(0, 'exfat', '\U0001f528', 'exFAT',
                    'Recommended \u2014 mountable image for most setups',
                    recommended=True,
                    tip='Recommended for most users. Easy to mount and '
                        'widely supported.')
    ...
    tk.Label(fmt_body,
             text='Not sure? exFAT is the safe default for most games.',
             font=FONTS['meta'], bg=COLORS['bg_2'], fg=COLORS['fg_4'],
             anchor='w').pack(fill='x', pady=(8, 0))

    # ── Advanced Options disclosure (PFS only) ──
    # The "intermediate format" is a build-pipeline internal that most users
    # should never touch (the exFAT default is almost always correct). It's
    # collapsed behind an Advanced Options header so it stays out of the
    # default beginner workflow but remains one click away for power users.
    adv_state = {'open': False}

    adv_disc_header = tk.Label(
        fmt_body, text='\u25b8  Advanced Options',
        font=FONTS['mono_sm'], bg=COLORS['bg_2'], fg=COLORS['fg_4'],
        cursor='hand2', anchor='w')

    via_frame = tk.Frame(fmt_body, bg=COLORS['bg_2'])
    tk.Label(via_frame, text='Intermediate format (advanced)',
             font=(FONTS['mono_sm'][0], 9, 'bold'), bg=COLORS['bg_2'],
             fg=COLORS['fg_2'], anchor='w').pack(anchor='w')
    ...
    _via_radios = tk.Frame(via_frame, bg=COLORS['bg_2'])
    _via_radios.pack(anchor='w', pady=(4, 0))
    for val, lab in (('exfat', 'exFAT (recommended)'), ('ffpkg', 'ffpkg')):
        tk.Radiobutton(_via_radios, text=lab, value=val, variable=pfs_via,
    ...
    tk.Label(via_frame,
             text='Determines the temporary format used before PFS '
                  'compression. Most users should leave this set to exFAT.',
    ...
    def _toggle_advanced():
        ...
        adv_disc_header.config(
            text=('\u25be  Advanced Options' if adv_state['open']
                  else '\u25b8  Advanced Options'))
        _refresh_via()

    adv_disc_header.bind('<Button-1>', lambda e: _toggle_advanced())
    ...
    # v3.6.3: cross-reference shown only in "Existing image" mode, so users
    # who actually want exFAT<->ffpkg conversion find the Convert tab.
    pfs_xref_lbl = tk.Label(
        fmt_body,
        text='Converting between exFAT and ffpkg? Use the Convert tab.',
        font=FONTS['meta'], bg=COLORS['bg_2'], fg=COLORS['fg_4'],
        anchor='w')

    def _refresh_via():
        # The "intermediate format" choice only applies when building a PFS
        # from a dump folder. Packing an existing image needs no intermediate,
        # so the disclosure is hidden in image mode. The control itself stays
        # collapsed behind "Advanced Options" until the user expands it.
        if out_type.get() == 'pfs':
            pfs_mode_frame.pack(anchor='w', pady=(12, 0))
        else:
            pfs_mode_frame.pack_forget()
        _show_adv = (out_type.get() == 'pfs'
                     and pfs_src_mode.get() == 'folder')
        if _show_adv:
            adv_disc_header.pack(anchor='w', pady=(10, 0))
            if adv_state['open']:
                via_frame.pack(anchor='w', pady=(6, 0), padx=(14, 0))
            else:
                via_frame.pack_forget()
        else:
            adv_disc_header.pack_forget()
            via_frame.pack_forget()
        # Show the Convert cross-reference only in PFS "Existing image" mode.
        if out_type.get() == 'pfs' and pfs_src_mode.get() == 'image':
            pfs_xref_lbl.pack(anchor='w', pady=(8, 0))
        else:
            pfs_xref_lbl.pack_forget()
    ...
    # ── Advanced Performance Options (collapsible) ───────────────────
    # v3.6.4: copy-threads + compression level moved off the main Build
    # surface into this disclosure. They are POWER-USER knobs with safe
    # defaults (threads 1, compression 9); most users never need them, so
    # they were cluttering the primary flow. NOTHING about their wiring
    ...
    apo_hdr = tk.Frame(apo_wrap, bg=COLORS['bg_3'],
                       highlightbackground=COLORS['border_2'],
    ...
    apo_body = tk.Frame(apo_wrap, bg=COLORS['bg_2'],
                        highlightbackground=COLORS['border_2'],
                        highlightthickness=1)

    def _apo_toggle(_e=None):
        apo_state['open'] = not apo_state['open']
        if apo_state['open']:
            apo_body.pack(fill='x')
            apo_caret.config(text='\u25be')
        else:
            apo_body.pack_forget()
            apo_caret.config(text='\u25b8')
    for _w in (apo_hdr, apo_caret):
    ...
    # two controls stay in sync. This controls FILE-COPY parallelism only;
    # PFS/mkpfs compression workers are a separate, memory-capped subsystem
    # (see _pfs_pack_tuning). Selecting a value clamps + persists via the
    # existing _adv_clamp_threads / _save_adv_params helpers.
    cpu_row = tk.Frame(apo_body, bg=COLORS['bg_2'])
    ...
    clvl_row = tk.Frame(apo_body, bg=COLORS['bg_2'])
    clvl_row.pack(fill='x', pady=(12, 12), padx=12)
    ...
    tip_lines = [
        ...
        ('Build copy threads affect the copy, not the squeeze.',
         'This selector controls parallel file copies during the build '
         '(robocopy /MT). PFS compression uses a separate, memory-safe '
         'worker count that is set automatically \u2014 it is not controlled '
         'by this setting.'),
    ]
        ...
        try:
            ...
            if bd is not None and bt:
                ...
                try:
                    ...
                    # v3.6.3: state the applied mkpfs worker count explicitly
                    # so the card never reads as if the Build copy threads
                    # setting was used here (it isn't — see _pfs_pack_tuning).
                    if t.get('pfs_workers'):
                        metric_vars['current_file'].set(
                            'Compression workers: %d (memory-safe limit)'
                            % t['pfs_workers'])
                except Exception:
                    pass
            ...
            if phase in (4, 5):
                ...
                if t.get('pfs_workers'):
                    bits.append('%d workers' % t['pfs_workers'])
                if bits:
    ...
    def _maybe_build_ampr_index(src_root, fmt_label=''):
        """v3.6.3: if the dump contains fakelib/libSceAmpr.sprx, generate an
        ampr_emu.index at the app0 root (src_root) so the AMPR file resolver
        can find the game's files (case-insensitively) after conversion.

        src_root IS the app0 root (the dump folder). The index is written as
        a sibling of eboot.bin / sce_sys / fakelib and is then packed into
        the built image by the normal copy/pack step. Gated strictly on the
        fakelib sprx being present — never generated otherwise. Best-effort:
        a failure here is logged but does NOT abort the build (the game just
        won't have the resolver index).
        """
        try:
            sprx = os.path.join(src_root, 'fakelib', 'libSceAmpr.sprx')
            if not os.path.isfile(sprx):
                app._log('[AMPR] libSceAmpr.sprx not found \u2014 skipping '
                         'index generation\n')
                return False
            app._log('[AMPR] libSceAmpr.sprx detected%s\n'
                     % (' (%s)' % fmt_label if fmt_label else ''))
            app._log('[AMPR] Generating ampr_emu.index\u2026\n')
            from ui.build_ampr_index import build_index_local
            from pathlib import Path
            out = Path(src_root) / 'ampr_emu.index'
            rc = build_index_local(Path(src_root), out, True)
            if rc == 0 and out.is_file():
                try:
                    _sz = out.stat().st_size
                except Exception:
                    _sz = 0
                app._log('[AMPR] ampr_emu.index written to app0 root '
                         '(%s, %d bytes)\n' % (out.name, _sz))
                return True
            else:
                app._log('[AMPR] index generation returned %s \u2014 '
                         'continuing build without it\n' % rc)
                return False
        except Exception as e:
            try:
                app._log('[AMPR] index generation error (%s) \u2014 '
                         'continuing build without it\n' % e)
            except Exception:
                pass
            return False
    ...
    def _dispatch_exfat(job, cb):
        try:
            # v3.6.3: AMPR index (gated on fakelib/libSceAmpr.sprx) — must be
            # written into the source/app0 root before the image is built so
            # it gets packed in. Covers exFAT directly, and PFS-via-exFAT
            # (which calls this with the same src).
            _maybe_build_ampr_index(job['src'], 'exFAT')
    ...
    def _dispatch_ffpkg(job, cb):
        try:
            # v3.6.3: ffpkg packs the whole source folder via
            # `UFS2Tool newfs -D <src>`, so writing the index into the source
            # root before the build lands it at the app0 root inside the
            # package — a clean pre-pack insertion point.
            if os.path.isfile(os.path.join(job['src'], 'fakelib',
                                           'libSceAmpr.sprx')):
                app._log('[AMPR] FFPKG staging path detected \u2014 '
                         'generating index\n')
            _maybe_build_ampr_index(job['src'], 'ffpkg')
    ...
    def _choose_intermediate_dir(via, outdir, src_folder):
        """v3.6.4 (issue #31 / #25 / #45): decide WHERE the Stage-A
        intermediate image (.exfat / .ffpkg) is built.

        Historically the intermediate was always created in `outdir`, so a
        user with temp=fast-SSD and output=slow-HDD paid the full
        image-build write cost (the heaviest, longest-running stage) on the
        slow drive. This picks the configured temp/staging folder for the
        intermediate when it is BOTH safe and actually useful, and falls
        back to `outdir` (the original behaviour) otherwise.

        Returns the directory to build the intermediate in, or None to
        abort the whole job (only when output itself can't hold the image,
        i.e. the build is doomed either way and we should fail early with a
        clear message rather than midway).

        Safety rules (deliberately conservative):
          * temp must be set and an existing directory;
          * temp must be on a DIFFERENT physical drive than output (same
            drive = zero benefit, so don't add a redirect);
          * temp must have enough free space for the estimated image plus
            headroom;
          * for via='ffpkg' ONLY, temp must also support hard links — the
            ffpkg/PFS staging path uses os.link, which fails on exFAT/FAT32.
            This is the historical "staging must support hard links" trap;
            via='exfat' is a plain OSFMount/robocopy file write and is safe
            on any temp drive, so it skips this check.
        Stage B (compression) is untouched: it still reads the intermediate
        wherever it is and writes the final .ffpfsc to `outdir` (we pass it
        an explicit output path below), and its own space guard decides
        whether to stage the compressed file on temp — so the two stages
        cooperate without this function having to reason about Stage B.
        """
        def _log(msg):
            try:
                app._log('[PFS] Intermediate image staging: %s\n' % msg)
            except Exception:
                pass

        temp = (app._settings.get('temp_dir') or '').strip()

        # Estimate the intermediate size from the source dump. The exFAT
        # intermediate is built 'auto' (smallest safe ~= source + margin),
        # so the source folder size is a sound, conservative basis.
        try:
            est = app._get_folder_size(src_folder) if src_folder else 0
        except Exception:
            est = 0
        # Headroom: image carries filesystem + allocator overhead on top of
        # the raw file bytes, so require comfortably more than the estimate.
        need_temp = int(est * 1.3) if est else 0
        # Output is only considered "doomed" (early-fail) if it can't even
        # hold roughly the raw image — a tighter bound than the temp guard
        # so we never false-abort a build output could actually fit.
        need_out_min = int(est * 1.05) if est else 0

        def _free(path):
            try:
                return _pfs_free_space(path)
            except Exception:
                return None

        # ── If temp can't be used, fall back to outdir (and early-fail only
        #    when outdir itself clearly lacks space). ──
        def _fallback(reason):
            _log('temp not used because %s.' % reason)
            of = _free(outdir)
            if est and (of is not None) and of < need_out_min:
                _log('falling back to output folder, but the output drive '
                     'has only %s free and the image needs about %s \u2014 '
                     'not enough space. Aborting before build.'
                     % (app._fmt_size(of), app._fmt_size(est)))
                return None
            _log('falling back to output folder %s.' % outdir)
            return outdir

        if not temp:
            return _fallback('no temp/staging folder is configured')
        if not os.path.isdir(temp):
            return _fallback('the configured temp folder does not exist (%s)'
                             % temp)

        # Same-drive check — no benefit redirecting within one physical drive.
        try:
            same_drive = (os.path.splitdrive(os.path.abspath(temp))[0].lower()
                          == os.path.splitdrive(os.path.abspath(outdir))[0].lower())
        except Exception:
            same_drive = True
        if same_drive:
            return _fallback('temp and output are on the same drive')

        # Conservative space guard on temp.
        if not est:
            return _fallback('the source image size could not be estimated, '
                             'so temp free space cannot be verified')
        tf = _free(temp)
        if tf is None:
            return _fallback('temp free space could not be read')
        if tf < need_temp:
            return _fallback('temp has %s free but the image needs about %s '
                             '(incl. headroom)'
                             % (app._fmt_size(tf), app._fmt_size(need_temp)))

        # ffpkg-only hard-link safety (exFAT path is a plain file write).
        if via == 'ffpkg':
            try:
                hl_ok = _pfs_supports_hardlinks(temp)
            except Exception:
                hl_ok = False
            if not hl_ok:
                return _fallback('the ffpkg intermediate needs a temp drive '
                                 'that supports hard links and this one does '
                                 'not (exFAT/FAT32?)')

        try:
            _fs = _pfs_fs_name(temp)
        except Exception:
            _fs = 'unknown'
        _log('using temp folder %s (%s, %s free).'
             % (temp, _fs, app._fmt_size(tf)))
        return temp
    ...
    def _dispatch_pfs(job, cb):
        ...
        # v3.6.4 (issue #31/#25/#45): stage the intermediate image on the
        # configured temp/staging folder when it's safe and useful, instead
        # of always writing it to the (possibly slow) output drive. The
        # final .ffpfsc location is UNCHANGED — still the output folder.
        inter_dir = _choose_intermediate_dir(via, job['outdir'], job['src'])
        if inter_dir is None:
            # Output drive can't hold the image either — fail early (the
            # helper already logged exactly why) rather than midway.
            try:
                app._set_status('PFS build aborted \u2014 not enough disk '
                                'space (see log)', '#d9534f')
            except Exception:
                pass
            cb(False)
            return
        inter_path = os.path.normpath(os.path.join(inter_dir, inter_name))
        final_path = os.path.normpath(os.path.join(job['outdir'], stem + '.ffpfsc'))
        inter_job = {'src': job['src'], 'outdir': inter_dir, 'name': inter_name}
        ...
        def _after_build(ok):
            ...
            try:
                # v3.6.4: the intermediate may now live on the temp drive,
                # so we can no longer let the pipeline infer the .ffpfsc
                # location from the image path ("beside the source") — that
                # would put the final file on temp. Pass the real output
                # target explicitly so the final .ffpfsc always lands in the
                # user's output folder. (allow_experimental is left at its
                # default False, so the experimental mkpfs branch — gated on
                # that flag — stays off exactly as before.)
                app._pipeline_convert_then_next(
                    inter_path, _after_convert,
                    explicit_output_path=final_path)
            except Exception as e:
                try:
    ...
    def _dispatch_pfs_from_image(job, cb):
        ...
        def _finish_after_convert(stage_dir, produced_ffpfsc):
            def _after_convert():
                ...
                if ok2:
                    try:
                        # Issue #39: hardened finalize. _move_final is a
                        # verified no-op when produced == final (the only
                        # case this route produces today), an atomic rename
                        # same-drive, and a verified copy cross-drive — and
                        # on ANY failure the produced file survives. Runs on
                        # the Tk thread, which is fine for the no-op/rename
                        # paths this site can hit.
                        out_final = app._move_final(
                            produced_ffpfsc, final_path,
                            log=lambda s: app._log('[UNIBUILD] %s\n' % s),
                            progress=_uq_set_progress)
                    except Exception as e:
                        app._log('[UNIBUILD] PFS(image): could not move to %s '
                                 '(%s) — keeping the produced file at %s.\n'
                                 % (final_path, e, produced_ffpfsc))
                ...
                if ok2:
                    try:
                        ...
                        pass
                if stage_dir:
                    # Issue #39 guard: never rmtree the staging folder if
                    # the finished file still lives INSIDE it (finalize
                    # failed and we kept the produced name). Deleting it
                    # here was the only path that could destroy a completed
                    # build after a failed move. On any doubt, keep it.
                    try:
                        _inside = os.path.normcase(
                            os.path.abspath(out_final)).startswith(
                            os.path.normcase(os.path.abspath(stage_dir))
                            + os.sep)
                    except Exception:
                        _inside = True
                    if _inside:
                        app._log('[UNIBUILD] PFS(image): finished file is '
                                 'inside the staging folder — leaving %s '
                                 'in place.\n' % stage_dir)
                    else:
