...
### Fixed
- **Extract tab showed the wrong cover art** when an `.exfat` was selected from
  a folder containing other extracted games. The Extract tab scanned the whole
  parent folder and used the first `icon0.png` it found, so a skull/face cover
  from a different title could appear. Cover art is now matched to the selected
  game: **Library Title ID first, then the exact sibling dump folder** (by image
  name or Title ID), then a placeholder. It never scans the parent folder.
  Build / Convert / Library cover art is unchanged.
- **Extraction progress now advances during multithread (`/MT`) extraction.**
  Progress had been parsed from robocopy's percentage output, which `/NP`
  suppresses, so the bar barely moved under `/MT`. Percentage and ETA are now
  driven by a destination-size poller (the same approach as the .ffpkg path),
  with a **"Scanning image…"** status shown during the initial size scan. The
  poller stops on success, failure, or cancellation.
- **exFAT build/convert: "OSFMount failed with exit code N" now preflights and
  explains itself (issue #42).** OSFMount allocates the full-size image
  container the moment it attaches, so an output drive without enough free
  space failed at attach with nothing but a bare OSFMount exit code. The build
  script now checks free space on the output drive (image size + 256 MB
  headroom) **before** mounting and fails with the actual numbers ("needs X,
  only Y free at Z") and a way out. When the attach still fails for another
  reason, OSFMount's own output is matched against known failure phrasings
  (out of space / access denied / driver not loaded / file or letter in use)
  and a likely-cause line is appended to the error instead of the bare exit
  code. The check uses `System.IO.DriveInfo` only (no WMI, per the
  media-detection hang history) and is skipped with a logged note on network
  shares / unmapped volumes, where the attach itself remains the backstop.
- **Added progress reporting for PFS unpack (`.ffpfs` / `.ffpfsc`) (issue #40).**
  mkpfs's CLI runs `unpack` without a progress reporter (unlike `pack`), so
  there were no progress lines to parse and the Extract bar sat at "Starting
  PFS unpack…" until completion. Progress is now driven by a destination-size
  poller — the same approach as the `.exfat` and `.ffpkg` extract routes —
  showing percent against the image size, bytes written, speed, and an
  estimated ETA, updating every 2 seconds. For a compressed `.ffpfsc` the
  decompressed output can exceed the image size, so percent caps at 99% until
  mkpfs finishes (the bytes/speed line stays accurate throughout); only the
  success path sets 100%. If a future mkpfs build emits unpack progress
  lines, they take precedence automatically — the poller only speaks after
  3 seconds of parser silence. This completes #40 across all three extract
  routes: `.exfat` (this release), `.ffpkg` (v3.6.2 engine), and PFS.
- **Library now detects uncompressed `.ffpfs` PFS images (issue #20).**
  The Library / My Images scanner recognized `.exfat`, `.ffpkg`, and
  `.ffpfsc` but skipped the uncompressed `.ffpfs` variant, so legacy and
  manually-packed PFS images never appeared. `.ffpfs` is now in the scan
  extension set, which covers all three filter sites at once: the local
  Library scan, the PS5-sync local pass, and the PS5 FTP listing. Cards
  label both `.ffpfs` and `.ffpfsc` as **PFS** (the display layer already
  supported this), and the empty-state hint lists all four formats. Note
  for the issue: Dump Rename scans game **dump folders** (via `eboot.bin`),
  not image files, so the Dump Rename half of the request doesn't apply —
  image files live in the Library.
- **Final-output moves are now cross-drive safe and verified (issue #39
  hardening).** A user report of `WinError 17` with a build that said
  complete while the final `.ffpfsc` was missing could not be reproduced in
  the current code, but the whole class is now structurally impossible. A
  new `_move_final` helper handles every final placement: same drive =
  atomic rename; cross drive (`WinError 17` / `EXDEV`) or any failed rename
  = chunked copy to `<name>.part` with progress, fsync, a **size verify
  against the source**, atomic rename into place — and the staged file is
  deleted only after the destination verifies. On any failure the completed
  file is left intact and the error states exactly where it is. Used by the
  unified Build tab's existing-image → `.ffpfsc` finalize (replacing a raw
  `os.replace`) and the pipeline's Temp-drive → output move-back. The
  Build-tab staging cleanup also gained a guard: it never deletes the
  staging folder if the finished file still lives inside it. Log lines:
  "Finalizing output", "Same-drive move complete" / "Cross-drive output
  detected, copying final image...", "Final output verified".
- **Dump → `.ffpfsc` now stages the intermediate image on the temp folder
  (issue #31 / #25 / #45).** Previously, building a dump folder to
  compressed `.ffpfsc` always created the intermediate exFAT/ffpkg image in
  the *output* directory, ignoring the configured temp/staging folder for
  that stage. With temp on a fast SSD/NVMe and output on a slow HDD, the
  heaviest, longest-running stage (the full image build — high disk active
  time and heavy writes) ran on the slow drive, producing "extremely slow"
  builds. The intermediate is now built on the temp folder when it's both
  safe and useful, decided by a new `_choose_intermediate_dir` guard:
  temp must exist, be on a *different* drive than output (same-drive gives
  no benefit), and have enough free space for the image plus headroom; for
  the **ffpkg** intermediate it must additionally support hard links (the
  exFAT/FAT32 `os.link` staging trap), while the **exFAT** intermediate —
  a plain OSFMount/robocopy file write — is safe on any temp drive.
  Otherwise it falls back to the output folder (the previous behaviour),
  and fails early with a clear message only if the output drive itself
  can't hold the image. Clear `[PFS] Intermediate image staging: …` log
  lines state which folder was chosen and why. **The final `.ffpfsc` still
  goes to the selected output folder, always** — the pipeline is now told
  the real output path explicitly so it never infers it from the (possibly
  temp) intermediate location. Stage B compression, its `--temp-folder`
  spool, its `.ffpfsc`-on-temp redirect + 2.2× space guard, and the #39
  `_move_final` data-safety contract are all unchanged; the intermediate is
  still deleted only after a verified-good compression, and a failed move
  never deletes the only finished file. Only `ui/tab_unibuild.py` changed.
- **Settings cleanup — clearer Build screen, no pipeline change (pre-4.0
  safe pass).** Four surgical UI/label changes, zero backend behaviour
  change: **(1)** removed the dead "Save robocopy log to file" controls
  from the Advanced tab — the toggle and its path field wrote settings
  (`adv_robo_log` / `adv_robo_log_dir`) that no build path ever consumed
  (no `/LOG` flag was ever passed to robocopy), so it was a control that
  did nothing; the backing vars are left defined for settings-compat.
  **(2)** Renamed the ambiguous "Retries (/R)" to "Robocopy file retry
  count (/R)" and clarified its help text, to distinguish it from the
  separate whole-build auto-retry (different layers — robocopy's per-file
  `/R` vs re-running the whole build). **(3)** Unified the copy-threads
  default: the parse-failure fallback in `_get_threads_arg` was 8 while
  the stored default everywhere else is 1, so a corrupt value behaved
  differently from a fresh one — both are now 1. **(4)** Moved the Build
  copy-threads and compression-level selectors off the main Build surface
  into a collapsed "Advanced performance options" disclosure; they're
  power-user knobs with safe defaults (threads 1, compression 9) that most
  users never touch. The controls bind the exact same vars and handlers as
  before — re-parenting only — so builds and persistence are identical.

### Changed
- **exFAT extraction Robocopy flags refined** to `/E /COPY:DAT /DCOPY:DA /R:2
  /W:2 /NP`: directory timestamps dropped (harmless for PS5 dumps, safer on
  virtual mounts), retry/wait tuned, and the inert `/ETA` removed. `/MT:N` is
  still added only when multithread extraction is enabled, and the classic
  single-thread copy remains the default fallback.
- **Build vs extraction copy-thread settings disambiguated.** The build setting
  "Concurrent Copy Threads" is now **"Build Copy Threads"**, and the extraction
  controls are **"Extraction multithread (/MT)"** and **"Extraction copy threads
  (/MT:N)"**, so the two are no longer confused. Default extraction thread count
  is unchanged (32).
